...
"""
    ...
    for configuration in get_all_configurations(arguments):
        ...
        run_command(arguments, ["sudo", "-u", "postgres", "-i", "psql", "-h", "pg", "-v", "ON_ERROR_STOP=1", "-c", "DROP DATABASE IF EXISTS %s;" % configuration.database.PARAMETERS["database"]])
        run_command(arguments, ["sudo", "-u", "postgres", "-i", "psql", "-h", "pg", "-v", "ON_ERROR_STOP=1", "-c", "DROP ROLE IF EXISTS %s;" % configuration.database.PARAMETERS["user"]])
